...
| 12.6 | Thème mail personnalisé (branding Kbeauty) | Backend | P2 | ⚪ | Tous les emails actuels (`OrderConfirmation`, `LowStockAlert`, ceux de Fortify) utilisent le thème markdown par défaut de Laravel (texte brut, sans logo ni couleurs). Publier le thème (`php artisan vendor:publish --tag=laravel-mail`) et personnaliser `resources/views/vendor/mail/html/theme/default.blade.php` (+ éventuellement `themes/default.css`) aux couleurs/logo du site. Impacte tous les emails existants et futurs (12.2-12.5, 13.2, 24.1). |
| 12.7 | Alerte stock bas (admin) | Backend | P1 | 🟢 | Déjà implémenté mais jamais documenté ici : `App\Notifications\LowStockAlert` (`ShouldQueue`), envoyée via `App\Application\Stock\UseCases\RecordStockMovement::notifyIfCrossingLowStockThreshold()` à tous les `User::role('admin')` au moment où le stock d'une variante franchit `config('inventory.low_stock_threshold')` à la baisse (déclenche une seule fois par franchissement, pas à chaque décrément sous le seuil). |
...
| 13.3 | Confirmation d'inscription newsletter (double opt-in) | Backend | P2 | ⚪ | Dépend de 13.2. Email de confirmation avec lien à cliquer avant que `consent_at` soit renseigné — évite qu'une adresse tierce soit inscrite sans son accord (recommandé RGPD). |
...
| # | Tâche | Dev | Priorité | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| 25.1 | Multi-langue (FR/EN) | Les deux | P2 | 🟡 | Infra posée + 2 sessions de traduction. **Infra :** middleware `locale:{fr,en}` (préfixe `/en`, groupe `Route::prefix('en')->name('en.')->middleware('locale:en')` en miroir de chaque route FR), locale partagée via Inertia, `laravel-react-i18n` côté front (`t()`/`tChoice()`, clés = phrase française, `lang/en.json`), `spatie/laravel-translatable` côté back (colonnes JSON `{"fr": ..., "en": ...}`, migration de backfill sans perte pour chaque colonne convertie). Bug de régression corrigé : la langue restait figée en changeant de page côté client — fixé via `I18nLocaleSync` dans `app.tsx` (générique, couvre toutes les pages). **Traduit :** page catalogue pilote (`/produits` ↔ `/en/produits`, `Product.name`/`description` traduisibles), puis extension au tunnel d'achat complet (fiche produit, panier, commande, paiement, confirmation — `short_description`/`ingredients_inci`/`how_to_use` rendus traduisibles en plus de `name`/`description`), sélecteur de langue extrait en composant partagé `components/language-switcher.tsx`. **Pas encore traduit (sessions suivantes) :** compte client (dashboard, adresses, historique commandes), auth/register/settings, pages légales (contenu encore `[À COMPLÉTER]` par endroits, à finaliser en français avant traduction), emails transactionnels, back-office admin (volontairement français). Tests Pest dédiés par route i18n (`CatalogLocaleTest.php` et équivalents pour le tunnel d'achat), vérifiant réponse 200 + `locale` correcte côté props Inertia + contenu traduit affiché. |
...
| 26.2 | Portail self-service retours/échanges | Les deux | P2 | ⚪ | S'appuie sur `orders`/`order_items`/`refunds` existants — à concevoir. Chaque changement de statut de la demande (soumise, acceptée, refusée) doit notifier le client par email ; la soumission doit aussi notifier un admin. |
...
| 26.5 | Formulaire de contact | Les deux | P1 | ⚪ | Actuellement absent de toute phase — page front simple (nom/email/message) + endpoint Laravel qui envoie un email à l'équipe (admin) via `Mail`/`Notification`. Pas de modèle back nécessaire (pas de table `contact_messages` à ce stade, juste un envoi direct), sauf si un historique des messages est souhaité plus tard. |
