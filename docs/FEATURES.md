...
| 12.9 | Notification admin : nouvel avis à modérer | Backend | P1 | ⚪ | Dépend de 14.1/15.1. Aucun admin n'est notifié qu'un avis attend une décision `approved`/`rejected` (15.2) — sans ça, la modération ne peut se faire qu'en allant vérifier manuellement l'admin. |
...
| 15.2 | Modération avis (admin) | Backend | P1 | ⚪ | Écran admin Inertia/React (16.1), pas Filament. `status` = `pending`/`approved`/`rejected`, un avis n'impacte `reviews_avg_rating` qu'une fois `approved`. |
...
| 26.2 | Portail self-service retours/échanges | Les deux | P2 | ⚪ | S'appuie sur `orders`/`order_items`/`refunds` existants — à concevoir. Chaque changement de statut de la demande (soumise, acceptée, refusée) doit notifier le client par email ; la soumission doit aussi notifier un admin (même besoin que 12.9). |
