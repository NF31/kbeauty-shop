...
| 16.2 | CRUD produits/catégories | Les deux | P0 | 🟡 | **Catégories faites.** `App\Http\Controllers\Admin\CategoryController` (resource, sans `show`) + `StoreCategoryRequest`/`UpdateCategoryRequest`, protégé par `permission:products.manage` (nouveau sous-groupe dans `routes/admin.php` — exclut `support`, qui n'a pas cette permission). Slug auto-généré via `spatie/laravel-sluggable` (attribut `#[Sluggable(from: 'name', to: 'slug')]` sur `Category`, cohérent avec le style `#[Fillable]`/`#[ObservedBy]` du projet — pas besoin du trait `HasSlug`) : régénéré à chaque changement de `name`, unique par suffixe numérique automatique. `Category::descendantIds()` empêche de déplacer une catégorie sous l'un de ses propres descendants (cycle dans l'arbre). Pages `pages/admin/categories/{index,create,edit}.tsx`, listing via `DataTable` (16.1). 8 tests Pest, Larastan clean. **Reste à faire (16.2b) :** `app/Http/Controllers/Admin/ProductController.php` + pages `pages/admin/products/{Index,Create,Edit}.tsx` — gestion brand/catégories/axes de variantes (4.2). L'upload Cloudinary (5.1) suivra dans une 3e branche une fois les produits en place. |
