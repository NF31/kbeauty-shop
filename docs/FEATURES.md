...
| 12.7 | Alerte stock bas (admin) | Backend | P1 | 🟢 | Déjà implémenté mais jamais documenté ici : `App\Notifications\LowStockAlert` (`ShouldQueue`), envoyée via `StockService::notifyIfCrossingLowStockThreshold()` à tous les `User::role('admin')` au moment où le stock d'une variante franchit `config('inventory.low_stock_threshold')` à la baisse (déclenche une seule fois par franchissement, pas à chaque décrément sous le seuil). |
