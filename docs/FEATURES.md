...
| 1.2 | Setup PostgreSQL + migrations de base | Backend | P0 | 🟢 | `DB_CONNECTION=pgsql` en local (Neon, connexion directe via `DB_URL`, pas de pooler `-pooler`/mode transaction — cohérent avec l'avertissement ci-dessous). `.env.example` mis à jour. Migrations de base (`users`, `cache`, `jobs`, `passkeys`, 2FA) passées avec succès sur PostgreSQL. |
