...
| # | Tâche | Dev | Priorité | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| 17.1 | Facturation PDF (dompdf) | Backend | P1 | ⚪ | `barryvdh/laravel-dompdf`, généré depuis les données `orders`/`order_items` déjà en snapshot (pas besoin de retourner sur le catalogue courant). À décider : PDF joint directement à l'email `OrderConfirmation` (12.2) ou lien de téléchargement depuis l'espace client (9.5) — probablement les deux. |
