...
| 12.8 | Notification admin : nouvelle commande payée | Backend | P1 | ⚪ | Manque actuellement : seul le client reçoit `OrderConfirmation` (12.2), aucun admin/staff n'est notifié qu'une commande vient d'être payée. Déclencher depuis `StripeWebhookController::markAsPaid()` (comme 12.2), envoyer à `User::role('admin')` (même pattern que 12.7). |
