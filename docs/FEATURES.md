...
| # | Tâche | Dev | Priorité | Statut | Détails |
| --- | --- | --- | --- | --- | --- |
| 14.1 | Compte Trustpilot Business + config domaine | — | P1 | ⚪ | Prérequis côté utilisateur, hors code : créer/configurer le Business Unit Trustpilot, obtenir le Business Unit ID et les clés API (invitations + widgets). |
| 14.2 | Widget TrustBox (fiche produit + pages clés) | Frontend | P1 | ⚪ | Intégration du script TrustBox Trustpilot sur les pages produit et éventuellement l'accueil, avec le Business Unit ID en config `.env`. |
| 14.3 | Invitation à laisser un avis post-achat | Backend | P1 | ⚪ | Déclenchée à la livraison/complétion de commande (transition `shipments.status = shipped` ou `orders.status = completed`, cf. 12.3) : appel à l'API Trustpilot Product/Service Reviews pour envoyer l'invitation, avec les `order_items` comme preuve d'achat. |
...
| 15.2 | Tracking événements Klaviyo (`Placed Order`/`Ordered Product`) | Backend | P1 | ⚪ | Envoi des événements standard Klaviyo depuis `PlaceOrder`/webhooks Stripe, nécessaires pour que Klaviyo puisse déclencher un flow de demande d'avis post-achat. |
