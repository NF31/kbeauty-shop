...
| 12.4 | Email vérification de compte | Backend | P1 | ⚪ | `MustVerifyEmail` déjà câblé sur `User` (Fortify) depuis 2.1, mais l'email natif (`Illuminate\Auth\Notifications\VerifyEmail`) n'a jamais été vérifié en conditions réelles ni testé depuis le passage à Resend (12.1). Vérifier l'envoi réel + ajouter un test Pest dédié. |
| 12.5 | Email reset password (vérification réelle) | Backend | P1 | ⚪ | Suite du point resté ouvert en 2.1 : email de reset (`Illuminate\Auth\Notifications\ResetPassword`, géré par Fortify) jamais revérifié en envoi réel depuis le passage à Resend (12.1). Fermer le point avec un envoi réel + test Pest. |
